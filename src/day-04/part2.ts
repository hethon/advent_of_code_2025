...
function updateGrid(grid: string[][], toRemove: Array<[number, number]>) {
...
let accessibleRolesOfPaperCount = 0;
while (true) {
	const accessibleRolesOfPaper = getAccessibleRolesOfPaper(grid);
	if (accessibleRolesOfPaper.length === 0) {
		break;
	}
	accessibleRolesOfPaperCount += accessibleRolesOfPaper.length;

	grid = updateGrid(grid, accessibleRolesOfPaper);
}
